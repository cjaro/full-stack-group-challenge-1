...
router.get('/warehouse', function(req, res) {
  pool.connect(function(err, client, done){
    if(err) {
      ...
      client.query('SELECT warehouse.fulfillment_days, warehouse.warehouse, products.description, warehouse_product.on_hand FROM warehouse JOIN warehouse_product ON warehouse.id = warehouse_product.warehouse_id JOIN products ON products.id = warehouse_product.product_id;', function(err, result){
        done();
        if(err) {
          console.log('Error making the database query: ', err);
          res.sendStatus(500);
